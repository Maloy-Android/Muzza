...
class Migration11To12 : AutoMigrationSpec {
    override fun onPostMigrate(database: SupportSQLiteDatabase) {
        ...
        database.execSQL("CREATE INDEX IF NOT EXISTS `index_song_albumId` ON `song` (`albumId`)")
    }
}
...
class Migration13To14 : AutoMigrationSpec {
    override fun onPostMigrate(db: SupportSQLiteDatabase) {
        ...
        val converters = Converters()
        val currentTime = converters.dateToTimestamp(LocalDateTime.now())
        db.execSQL("UPDATE playlist SET createdAt = $currentTime WHERE createdAt IS NULL")
        db.execSQL("UPDATE playlist SET lastUpdateTime = $currentTime WHERE lastUpdateTime IS NULL")
    }
}