package com.maloy.muzza.lyrics.simpmusic

import com.maloy.muzza.lyrics.simpmusic.models.LyricsData
import com.maloy.muzza.lyrics.simpmusic.models.SimpMusicApiResponse
    ...
    suspend fun getAllLyrics(
        ...
        sortedTracks.forEach { track ->
            if (count <= 4) {
                ...
                if (!track.richSyncLyrics.isNullOrBlank() && durationMatch) {
                    count++
                    callback(track.richSyncLyrics)
                } else if (!track.syncedLyrics.isNullOrBlank() && durationMatch) {
                    count++
                    callback(track.syncedLyrics)
                }
                if (!track.plainLyrics.isNullOrBlank() && durationMatch && plain == 0) {
                    count++
                    plain++
